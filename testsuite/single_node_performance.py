...
DEFAULT_NUM_INIT_ACCOUNTS = (
    "100000000" if SELECTED_FLOW == Flow.PREVIEWNET_LARGE_DB else "80000000"
)
DEFAULT_MAX_BLOCK_SIZE = "25000" if IS_PREVIEWNET else "2000"

MAX_BLOCK_SIZE = int(os.environ.get("MAX_BLOCK_SIZE", default=DEFAULT_MAX_BLOCK_SIZE))
NUM_BLOCKS = int(os.environ.get("NUM_BLOCKS_PER_TEST", default=100))
NUM_BLOCKS_DETAILED = 10
NUM_ACCOUNTS = max(
    [
        int(os.environ.get("NUM_INIT_ACCOUNTS", default=DEFAULT_NUM_INIT_ACCOUNTS)),
        (2 + 3 * NUM_BLOCKS) * MAX_BLOCK_SIZE,
    ]
)
MAIN_SIGNER_ACCOUNTS = NUM_BLOCKS * MAX_BLOCK_SIZE
...
TESTS = [
    ...
    RunGroupConfig(expected_tps=27, key=RunGroupKey("loop100k"), included_in=LAND_BLOCKING_AND_C | Flow.REPRESENTATIVE),
    RunGroupConfig(expected_tps=41, key=RunGroupKey("loop10k-arithmetic"), included_in=LAND_BLOCKING_AND_C | Flow.REPRESENTATIVE),
    RunGroupConfig(expected_tps=167, key=RunGroupKey("loop1k-bcs1k"), included_in=LAND_BLOCKING_AND_C | Flow.REPRESENTATIVE),
    RunGroupConfig(expected_tps=949, key=RunGroupKey("create-objects10"), included_in=LAND_BLOCKING_AND_C | Flow.REPRESENTATIVE),
    RunGroupConfig(expected_tps=103, key=RunGroupKey("create-objects10-with-payload10k"), included_in=LAND_BLOCKING_AND_C | Flow.REPRESENTATIVE),
    RunGroupConfig(expected_tps=142, key=RunGroupKey("create-objects100"), included_in=LAND_BLOCKING_AND_C | Flow.REPRESENTATIVE),
    RunGroupConfig(expected_tps=11, key=RunGroupKey("create-objects100-with-payload10k"), included_in=LAND_BLOCKING_AND_C | Flow.REPRESENTATIVE),
    ...
    RunGroupConfig(expected_tps=1500, key=RunGroupKey("vector-picture-create40"), included_in=Flow.CONTINUOUS, waived=True),
    RunGroupConfig(expected_tps=1500, key=RunGroupKey("vector-picture-create40", module_working_set_size=20), included_in=Flow.CONTINUOUS, waived=True),
    RunGroupConfig(expected_tps=1500, key=RunGroupKey("vector-picture40"), included_in=Flow.CONTINUOUS, waived=True),
    RunGroupConfig(expected_tps=1500, key=RunGroupKey("vector-picture40", module_working_set_size=20), included_in=Flow.CONTINUOUS, waived=True),
    RunGroupConfig(expected_tps=1500, key=RunGroupKey("vector-picture-read40"), included_in=Flow.CONTINUOUS, waived=True),
    RunGroupConfig(expected_tps=1500, key=RunGroupKey("vector-picture-read40", module_working_set_size=20), included_in=Flow.CONTINUOUS, waived=True),
    ...
    RunGroupConfig(expected_tps=30, key=RunGroupKey("vector-picture-create30k"), included_in=LAND_BLOCKING_AND_C),
    RunGroupConfig(expected_tps=29, key=RunGroupKey("vector-picture-create30k", module_working_set_size=20), included_in=LAND_BLOCKING_AND_C),
    RunGroupConfig(expected_tps=170, key=RunGroupKey("vector-picture30k"), included_in=LAND_BLOCKING_AND_C),
    RunGroupConfig(expected_tps=170, key=RunGroupKey("vector-picture30k", module_working_set_size=20), included_in=LAND_BLOCKING_AND_C),
    RunGroupConfig(expected_tps=200, key=RunGroupKey("vector-picture-read30k"), included_in=Flow.CONTINUOUS),
    RunGroupConfig(expected_tps=200, key=RunGroupKey("vector-picture-read30k", module_working_set_size=20), included_in=Flow.CONTINUOUS),
...
with tempfile.TemporaryDirectory() as tmpdirname:
    ...
    create_db_command = f"{BUILD_FOLDER}/aptos-executor-benchmark --block-size {MAX_BLOCK_SIZE} --execution-threads {NUMBER_OF_EXECUTION_THREADS} {DB_CONFIG_FLAGS} {DB_PRUNER_FLAGS} create-db --data-dir {tmpdirname}/db --num-accounts {NUM_ACCOUNTS} --init-account-balance 1000000000000000"
